...
-- ***** your code below *****
FROM events
WHERE patient_id NOT IN (SELECT patient_id FROM mortality WHERE patient_id is NOT NULL);
...
-- ***** your code below *****
FROM events
WHERE patient_id IN (SELECT patient_id FROM mortality WHERE patient_id is NOT NULL);
...
-- ***** your code below *****
FROM (SELECT count(event_id) as event_count FROM alive_events GROUP BY patient_id) eventcounts;
...
SELECT avg(event_count), min(event_count), max(event_count)
-- ***** your code below *****
FROM (SELECT count(event_id) as event_count FROM dead_events GROUP BY patient_id) deadcounts;
...
-- ***** your code below *****
FROM (SELECT count(DISTINCT to_date(time)) as encounter_count FROM alive_events GROUP BY patient_id) encounteralive; 
...
SELECT avg(encounter_count), min(encounter_count), max(encounter_count)
-- ***** your code below *****
FROM (SELECT count(DISTINCT to_date(time)) as encounter_count FROM dead_events GROUP BY patient_id) encounterdead; 
...
-- ***** your code below *****
FROM (SELECT datediff(max(to_date(time)), min(to_date(time))) AS record_length FROM alive_events GROUP BY patient_id) recordalive;
...
SELECT avg(record_length), percentile(record_length, 0.5), min(record_length), max(record_length)
-- ***** your code below *****
FROM (SELECT datediff(max(time), min(time)) AS record_length FROM dead_events GROUP BY patient_id) recorddead;

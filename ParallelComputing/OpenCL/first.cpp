...
void array_mul_reduction()
{
    ...
    cl_mem dD = clCreateBuffer( context, CL_MEM_READ_WRITE, dataSize, NULL, &status );
    if( status != CL_SUCCESS )
        fprintf( stderr, "clCreateBuffer failed (3)\n" );
    ...
    cl_kernel kernel = clCreateKernel( program, "ArrayMultRec", &status );
    if( status != CL_SUCCESS )
        fprintf( stderr, "clCreateKernel failed\n" );
    ...
    status = clSetKernelArg( kernel, 4, sizeof(float) * LOCAL_SIZE, NULL );
    if( status != CL_SUCCESS )
    ...
    status = clEnqueueReadBuffer( cmdQueue, dC, CL_TRUE, 0, sizeof(float), &hD, 0, NULL, NULL );
    if( status != CL_SUCCESS )
        fprintf( stderr, "clEnqueueReadBuffer failed\n" );
    ...
    /* reduction */
    for( int i = 1; i < NUM_WORK_GROUPS; i++ ) hC[0] += hC[i];
    ...
    if( fabs( hC[0] - expected ) > TOL )
    {
        fprintf( stderr, "wrongly produced %13.6f instead of %13.6f (%13.8f)\n",
                hC[0], expected, fabs(hC[0]-expected) );
    }
    printf("gpu: %f\n", hD);
    printf("cpu, reduction: %f\n", hC[0]);
...
{
    /* case 1 */
    printf("Case1: Multiply\n");
    array_mul();

    /* case 2 */
    printf("Case2: Multiply Add\n");
    array_mul_add();

    /* case 3 */
    printf("Case3: Multiply reduce\n");
    array_mul_reduction();
}

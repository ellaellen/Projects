...
icustays = icustays[icustays.LOS <= 2] # within 48hr

patients_icustays_48hr = icustays.groupby(['SUBJECT_ID']).count().reset_index()

patients_icustays_48hr_single = patients_icustays_48hr[patients_icustays_48hr.LOS > 1]


patients_train, patients_test = train_test_split(patients_icustays_48hr, test_size = 0.2, random_state = 6250)
...
patients_48hr_train = pd.merge(patients, patients_id_train)
patients_48hr_validation = pd.merge(patients, patients_id_validation)
patients_48hr_test = pd.merge(patients, patients_id_test)


mortality_train = patients_48hr_train[['SUBJECT_ID', 'EXPIRE_FLAG']]
mortality_train.columns = [['SUBJECT_ID', 'MORTALITY']]

mortality_test = patients_48hr_test[['SUBJECT_ID', 'EXPIRE_FLAG']]
mortality_test.columns = [['SUBJECT_ID', 'MORTALITY']]

mortality_validation = patients_48hr_validation[['SUBJECT_ID', 'EXPIRE_FLAG']]
